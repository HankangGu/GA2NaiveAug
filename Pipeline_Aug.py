...
def get_in_roads(in_lanes):
    in_roads=[]
    for lane in in_lanes:
        road = lane[:-2]
        if road not in in_roads:
...
def build_support_info(itsx_assignment, movements):
    ...
    for assignment in itsx_assignment:
        for itsx_id in assignment:
            ...
            # build lane-level adj on the same road
            in_roads= get_in_roads(movements[itsx_id])
            for in_road in in_roads:
                left_lane_id = in_road+"_0"
                straight_lane_id = in_road+"_1"
                right_lane_id = in_road+"_2"
                movement_graph.add_edge(left_lane_id,straight_lane_id)
                movement_graph.add_edge(left_lane_id,right_lane_id)
                movement_graph.add_edge(straight_lane_id,right_lane_id)
