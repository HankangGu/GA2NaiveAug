...
import copy
from Pipeline_Naive import pipeline
import json
...
def init_exp(args):
    ...
    # Construct Cityflow Configuration File and ENV DICT98
    ...
    # with open(os.path.join(ENV_CONFIG["PATH_TO_WORK_DIRECTORY"], "exp_config.json"), "w") as f:
    ...
    if EXP_CONFIG["REGIONAL"]:
        # @TODO merge obs dim and exu dim
    ...
    # else:
    #     agents = [EXP_CONFIG["AGENT_CLASS_DICT"][agent_type](i,agent_config) for i in range(EXP_CONFIG["AGETN_NUM"])]
    print(EXP_CONFIG)
    print(ENV_CONFIG)
...
if __name__ == "__main__":
    ...
    for i in range (4):
        args = parse_args()
        args.folder = "records/GAABDQNaive/"
        args.netname = netname_list[i]
        args.netshape = netshape_list[i]
        args.flow = flow_list[i]
        env,agent,itsx_assignment,EXP_CONFIG,ENV_CONFIG=init_exp(args)
